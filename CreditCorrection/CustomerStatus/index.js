...
export default class CustomerStatusLayout extends Component {
    ...
    // createCustomerStatusTransclusion() {
    //     const {country, storeNumber, customerNumber} = this.buildCustomerInfoData();
    //     const url = `/history/fragments/customers/${country}/${storeNumber}/${customerNumber}/recentrequests`;
    //    // the recent history includes a progress bar if there is a pending request which was already submitted
    //     return <Transclusion src={this.props.isTablet ? url + '?tablet=true' : url} />;
    // }
    getCustomer() {
        if (!this.props.customers || !this.props.customers.data || !this.props.customers.data.customers) {
            return null;
        }
    ...
    render() {
        ...
        if (this.props.customers.data.requestsDisabled) {
            btnDisabled = true;
        }
            ...
            <Switch>
                <Route
                    ...
                    render={() => {
                        return (
                            <MainContent>
                                <CustomerDataGroup customers={this.props.customers.data.customers} />
                            </MainContent>
                        );
                    }}
                />
...
CustomerStatusLayout.propTypes = {
    ...
    loadRecentRequests: PropTypes.func,
