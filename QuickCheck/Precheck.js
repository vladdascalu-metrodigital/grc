...
export default class Precheck extends Component {
    ...
    componentDidMount() {
        if (this.props.customers && this.props.customers.data) {
            ...
            this.setState({ qcrData: qcrNewData });

            // List<String> errorStrings = precheckErrors.stream()
            //     .map(p -> (String) p.get("reason"))
            //     .filter(r -> !"strategy.init.failed.runningRequest".equals(r))
    ...
    static renderErrors(data) {
        if (!data || !data.precheckErrors || data.precheckErrors.length <= 0) return null;
    ...
    renderErrorDetails(data) {
        if (!data || !data.precheckErrors) return null;
    ...
    render() {
        if (!this.state.qcrData) return null;
        const data = this.state.qcrData;
        const errorElements = Precheck.renderErrors(this.state.qcrData);
        return (
