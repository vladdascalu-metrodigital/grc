import React from 'react';
import { storiesOf } from '@storybook/react';
import AdditionalFieldsSection from '../AdditionalFieldsNew/AdditionalFieldsSection';

import { selection, selectionNoPreselection, multiSelection } from './fixtures/credit-limit/additionalfields.request';

import '../AdditionalFieldsNew/index.scss';

storiesOf('AdditionalFieldsNew/Single Fields', module)
    .add('Selection', () => {
        return <AdditionalFieldsSection requestFields={[selectionNoPreselection]} editable={true} />;
    })
    .add('Selection, Preselected', () => {
        return <AdditionalFieldsSection requestFields={[selection]} editable={true} />;
    })
    .add('Multi Selection, Preselected', () => {
        return <AdditionalFieldsSection requestFields={[multiSelection]} editable={true} />;
    })
    .add('Multi Selection, No Edit', () => {
        return <AdditionalFieldsSection requestFields={[multiSelection]} editable={false} />;
    });

storiesOf('AdditionalFieldsNew', module)
    .add('all types editable AdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: '98e541fc-ca74-48d1-8072-e2d00dce5238',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710225Z',
                        value: 'true',
                        textValue: null,
                        countryField: {
                            id: 'cb45aef4-bbe7-4f1d-95d2-ece99a148a1d',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'a6c4d42a-43db-400b-a515-077a327be619',
                                code: 'DMY_CUSTOMER_IS_PROFITABLE',
                                label: 'mrc.customer.is.profitable',
                                type: 'CHECKBOX',
                            },
                        },
                    },
                    {
                        id: '1dee6fe1-8168-4397-84e8-c02e609e0f97',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: null,
                        customerNumber: null,
                        groupId: null,
                        creationTimestamp: '2020-06-19T17:54:47.886469Z',
                        value: '2020-02-10',
                        textValue: null,
                        countryField: {
                            id: '5bc3b8e6-d29d-475d-bf89-01efcbcfec18',
                            mandatory: true,
                            validation: null,
                            level: 'REQUEST',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '3eb15d56-51eb-4fc0-bd80-ee52cb930e58',
                                code: 'FIELD_DATE_1',
                                label: 'field.date.one',
                                type: 'DATE',
                            },
                        },
                    },
                    {
                        id: '65145d2f-0fdc-4a4f-9827-bbd6d2a45f78',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709931Z',
                        value: 'VERY_IMPORTANT',
                        textValue: null,
                        countryField: {
                            id: '4cc61cef-80eb-44ab-850b-a7899e566629',
                            mandatory: false,
                            validation: null,
                            level: 'REQUEST',
                            order: 3,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: 'NOT_IMPORTANT|IMPORTANT|VERY_IMPORTANT',
                            field: {
                                id: 'a09bd711-d0f1-4900-8672-37acf6b983e8',
                                code: 'DMY_REQUEST_IMPORTANCE',
                                label: 'mrc.request.importance',
                                type: 'DROPDOWN',
                            },
                        },
                    },
                    {
                        id: 'fb0a75ff-0d99-4571-95db-161e90e0e107',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710184Z',
                        value: 'Computer_peripheral_equipment|Recreational_Products_Toys',
                        textValue: null,
                        countryField: {
                            id: 'e3992ab5-425c-45ed-99f6-c82f63ea9be0',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 4,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options:
                                'Oil_Gas_Production|Computer_peripheral_equipment|Recreational_Products_Toys|Electric_Utilities_Central|Package_Goods_Cosmetics|EDP_Services|Commercial_Banks|Metal_Fabrications|Industrial_Machinery_Components',
                            field: {
                                id: '2bcc9833-1aa8-40ed-8b31-985a28fa906e',
                                code: 'DMY_CUSTOMER_INDUSTRY',
                                label: 'mrc.customerdetails.industry',
                                type: 'DROPDOWN_MULTIPLE',
                            },
                        },
                    },
                    {
                        id: '05c4e967-e4db-4f36-91e5-69f1f970e8e4',
                        requestId: 'a9a6f85e-6180-42d1-ad5e-cc26b8f78d0c',
                        country: 'DE',
                        storeNumber: '15',
                        customerNumber: '116102',
                        groupId: '951951951951951',
                        creationTimestamp: '2020-06-19T13:59:02.313021Z',
                        value: '9',
                        textValue: null,
                        countryField: {
                            id: 'cc8cc3b2-da48-4962-adda-d208ab063856',
                            mandatory: false,
                            validation: 'isPercentage',
                            level: 'CUSTOMER',
                            order: 5,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'f7722295-085b-41ff-bd5a-981a07220b9e',
                                code: 'percprofitability',
                                label: 'percentage.profitability',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'd1b6368c-d8ad-4182-a9ab-e1a8352c7ecc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709544Z',
                        value: '123',
                        textValue: null,
                        countryField: {
                            id: '0660f875-b508-4f66-9cc7-141f1e1a962b',
                            mandatory: false,
                            validation: 'isNumber',
                            level: 'GROUP',
                            order: 5,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'c2a83b05-e05e-40e7-bfa9-8fd5835e4b94',
                                code: 'DMY_GROUP_TOTAL_TURNOVER',
                                label: 'mrc.mdw.totalturnover',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'b67a7132-4b25-4fd1-8408-88558a164711',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709889Z',
                        value: '2',
                        textValue: null,
                        countryField: {
                            id: 'ede80e37-7718-46b0-8566-eddcef681161',
                            mandatory: false,
                            validation: null,
                            level: 'GROUP',
                            order: 6,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: '1|2|3|4|5',
                            field: {
                                id: 'a0ca6a5a-2783-448f-a9c7-aefcd5b23f75',
                                code: 'DMY_GROUP_GENERAL_RATING',
                                label: 'mrc.recommendations.rating',
                                type: 'RADIOBUTTON',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
                editable={true}
            />
        );
    })
    .add('all types not editable AdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: '98e541fc-ca74-48d1-8072-e2d00dce5238',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710225Z',
                        value: 'true',
                        textValue: null,
                        countryField: {
                            id: 'cb45aef4-bbe7-4f1d-95d2-ece99a148a1d',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'a6c4d42a-43db-400b-a515-077a327be619',
                                code: 'DMY_CUSTOMER_IS_PROFITABLE',
                                label: 'mrc.customer.is.profitable',
                                type: 'CHECKBOX',
                            },
                        },
                    },
                    {
                        id: '1dee6fe1-8168-4397-84e8-c02e609e0f97',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: null,
                        customerNumber: null,
                        groupId: null,
                        creationTimestamp: '2020-06-19T17:54:47.886469Z',
                        value: null,
                        textValue: null,
                        countryField: {
                            id: '5bc3b8e6-d29d-475d-bf89-01efcbcfec18',
                            mandatory: true,
                            validation: null,
                            level: 'REQUEST',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '3eb15d56-51eb-4fc0-bd80-ee52cb930e58',
                                code: 'FIELD_DATE_1',
                                label: 'field.date.one',
                                type: 'DATE',
                            },
                        },
                    },
                    {
                        id: '65145d2f-0fdc-4a4f-9827-bbd6d2a45f78',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709931Z',
                        value: 'VERY_IMPORTANT',
                        textValue: null,
                        countryField: {
                            id: '4cc61cef-80eb-44ab-850b-a7899e566629',
                            mandatory: false,
                            validation: null,
                            level: 'REQUEST',
                            order: 3,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: 'NOT_IMPORTANT|IMPORTANT|VERY_IMPORTANT',
                            field: {
                                id: 'a09bd711-d0f1-4900-8672-37acf6b983e8',
                                code: 'DMY_REQUEST_IMPORTANCE',
                                label: 'mrc.request.importance',
                                type: 'DROPDOWN',
                            },
                        },
                    },
                    {
                        id: 'fb0a75ff-0d99-4571-95db-161e90e0e107',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710184Z',
                        value: 'Computer_peripheral_equipment|Recreational_Products_Toys',
                        textValue: null,
                        countryField: {
                            id: 'e3992ab5-425c-45ed-99f6-c82f63ea9be0',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 4,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options:
                                'Oil_Gas_Production|Computer_peripheral_equipment|Recreational_Products_Toys|Electric_Utilities_Central|Package_Goods_Cosmetics|EDP_Services|Commercial_Banks|Metal_Fabrications|Industrial_Machinery_Components',
                            field: {
                                id: '2bcc9833-1aa8-40ed-8b31-985a28fa906e',
                                code: 'DMY_CUSTOMER_INDUSTRY',
                                label: 'mrc.customerdetails.industry',
                                type: 'DROPDOWN_MULTIPLE',
                            },
                        },
                    },
                    {
                        id: '05c4e967-e4db-4f36-91e5-69f1f970e8e4',
                        requestId: 'a9a6f85e-6180-42d1-ad5e-cc26b8f78d0c',
                        country: 'DE',
                        storeNumber: '15',
                        customerNumber: '116102',
                        groupId: '951951951951951',
                        creationTimestamp: '2020-06-19T13:59:02.313021Z',
                        value: '9',
                        textValue: null,
                        countryField: {
                            id: 'cc8cc3b2-da48-4962-adda-d208ab063856',
                            mandatory: false,
                            validation: 'isPercentage',
                            level: 'CUSTOMER',
                            order: 5,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'f7722295-085b-41ff-bd5a-981a07220b9e',
                                code: 'percprofitability',
                                label: 'percentage.profitability',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'd1b6368c-d8ad-4182-a9ab-e1a8352c7ecc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709544Z',
                        value: '123',
                        textValue: null,
                        countryField: {
                            id: '0660f875-b508-4f66-9cc7-141f1e1a962b',
                            mandatory: false,
                            validation: 'isNumber',
                            level: 'GROUP',
                            order: 5,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'c2a83b05-e05e-40e7-bfa9-8fd5835e4b94',
                                code: 'DMY_GROUP_TOTAL_TURNOVER',
                                label: 'mrc.mdw.totalturnover',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'b67a7132-4b25-4fd1-8408-88558a164711',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709889Z',
                        value: '2',
                        textValue: null,
                        countryField: {
                            id: 'ede80e37-7718-46b0-8566-eddcef681161',
                            mandatory: false,
                            validation: null,
                            level: 'GROUP',
                            order: 6,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: '1|2|3|4|5',
                            field: {
                                id: 'a0ca6a5a-2783-448f-a9c7-aefcd5b23f75',
                                code: 'DMY_GROUP_GENERAL_RATING',
                                label: 'mrc.recommendations.rating',
                                type: 'RADIOBUTTON',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
                editable={false}
            />
        );
    })
    .add('editable requestAdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: '1dee6fe1-8168-4397-84e8-c02e609e0f97',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: null,
                        customerNumber: null,
                        groupId: null,
                        creationTimestamp: '2020-06-19T17:54:47.886469Z',
                        value: null,
                        textValue: null,
                        countryField: {
                            id: '5bc3b8e6-d29d-475d-bf89-01efcbcfec18',
                            mandatory: true,
                            validation: null,
                            level: 'REQUEST',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '3eb15d56-51eb-4fc0-bd80-ee52cb930e58',
                                code: 'FIELD_DATE_1',
                                label: 'field.date.one',
                                type: 'DATE',
                            },
                        },
                    },
                    {
                        id: '1644e283-036c-49d9-86a9-9eebc8695333',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709973Z',
                        value: null,
                        textValue:
                            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.",
                        countryField: {
                            id: 'c4ff9118-2886-4461-9037-59a530794a56',
                            mandatory: false,
                            validation: 'maxLength255',
                            level: 'REQUEST',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '3e112e86-4c0b-4c0f-b014-4a586b067dd7',
                                code: 'DMY_REQUEST_COMMENT',
                                label: 'mrc.request.comment',
                                type: 'TEXTAREA',
                            },
                        },
                    },
                    {
                        id: '65145d2f-0fdc-4a4f-9827-bbd6d2a45f78',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709931Z',
                        value: 'VERY_IMPORTANT',
                        textValue: null,
                        countryField: {
                            id: '4cc61cef-80eb-44ab-850b-a7899e566629',
                            mandatory: false,
                            validation: null,
                            level: 'REQUEST',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: 'NOT_IMPORTANT|IMPORTANT|VERY_IMPORTANT',
                            field: {
                                id: 'a09bd711-d0f1-4900-8672-37acf6b983e8',
                                code: 'DMY_REQUEST_IMPORTANCE',
                                label: 'mrc.request.importance',
                                type: 'DROPDOWN',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
                title={'additional.fields.request.title'}
                editable={true}
            />
        );
    })
    .add('not editable requestAdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: '1dee6fe1-8168-4397-84e8-c02e609e0f97',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: null,
                        customerNumber: null,
                        groupId: null,
                        creationTimestamp: '2020-06-19T17:54:47.886469Z',
                        value: null,
                        textValue: null,
                        countryField: {
                            id: '5bc3b8e6-d29d-475d-bf89-01efcbcfec18',
                            mandatory: true,
                            validation: null,
                            level: 'REQUEST',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '3eb15d56-51eb-4fc0-bd80-ee52cb930e58',
                                code: 'FIELD_DATE_1',
                                label: 'field.date.one',
                                type: 'DATE',
                            },
                        },
                    },
                    {
                        id: '1644e283-036c-49d9-86a9-9eebc8695333',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709973Z',
                        value: null,
                        textValue:
                            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.",
                        countryField: {
                            id: 'c4ff9118-2886-4461-9037-59a530794a56',
                            mandatory: false,
                            validation: 'maxLength255',
                            level: 'REQUEST',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '3e112e86-4c0b-4c0f-b014-4a586b067dd7',
                                code: 'DMY_REQUEST_COMMENT',
                                label: 'mrc.request.comment',
                                type: 'TEXTAREA',
                            },
                        },
                    },
                    {
                        id: '65145d2f-0fdc-4a4f-9827-bbd6d2a45f78',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709931Z',
                        value: 'VERY_IMPORTANT',
                        textValue: null,
                        countryField: {
                            id: '4cc61cef-80eb-44ab-850b-a7899e566629',
                            mandatory: false,
                            validation: null,
                            level: 'REQUEST',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: 'NOT_IMPORTANT|IMPORTANT|VERY_IMPORTANT',
                            field: {
                                id: 'a09bd711-d0f1-4900-8672-37acf6b983e8',
                                code: 'DMY_REQUEST_IMPORTANCE',
                                label: 'mrc.request.importance',
                                type: 'DROPDOWN',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
                title={'additional.fields.request.title'}
            />
        );
    })
    .add('editable groupAdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: 'd1b6368c-d8ad-4182-a9ab-e1a8352c7ecc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709544Z',
                        value: '123',
                        textValue: null,
                        countryField: {
                            id: '0660f875-b508-4f66-9cc7-141f1e1a962b',
                            mandatory: false,
                            validation: 'isNumber',
                            level: 'GROUP',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'c2a83b05-e05e-40e7-bfa9-8fd5835e4b94',
                                code: 'DMY_GROUP_TOTAL_TURNOVER',
                                label: 'mrc.mdw.totalturnover',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'a0b9c878-bb8e-435f-8385-a5c5695f25fc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710014Z',
                        value: 'example of string value',
                        textValue: null,
                        countryField: {
                            id: '5f36fbc7-c1c7-44df-a8df-fc0ea90c42b7',
                            mandatory: false,
                            validation: 'maxLength255',
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'eef491e5-466d-4f97-bf17-ea68208d1a73',
                                code: 'DMY_CUSTOMER_COMPANY_NAME',
                                label: 'mrc.customerdetails.fields.companyname',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '99efe89f-f397-4050-8205-26f8d080b2ef',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709854Z',
                        value: '20',
                        textValue: null,
                        countryField: {
                            id: '2e6cb78c-e5a9-49cc-93db-6a3e4db10990',
                            mandatory: false,
                            validation: 'isNumber',
                            level: 'GROUP',
                            order: 3,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'bbc8c7e4-b9d6-46b4-844b-9eca65829773',
                                code: 'DMY_GROUP_LIMIT_AVERAGE',
                                label: 'mrc.limit.average',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'b67a7132-4b25-4fd1-8408-88558a164711',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709889Z',
                        value: '2',
                        textValue: null,
                        countryField: {
                            id: 'ede80e37-7718-46b0-8566-eddcef681161',
                            mandatory: false,
                            validation: null,
                            level: 'GROUP',
                            order: 4,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: '1|2|3|4|5',
                            field: {
                                id: 'a0ca6a5a-2783-448f-a9c7-aefcd5b23f75',
                                code: 'DMY_GROUP_GENERAL_RATING',
                                label: 'mrc.recommendations.rating',
                                type: 'RADIOBUTTON',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
                title={'additional.fields.group.title'}
                editable={true}
            />
        );
    })
    .add('not editable groupAdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: 'd1b6368c-d8ad-4182-a9ab-e1a8352c7ecc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709544Z',
                        value: '123',
                        textValue: null,
                        countryField: {
                            id: '0660f875-b508-4f66-9cc7-141f1e1a962b',
                            mandatory: false,
                            validation: 'isNumber',
                            level: 'GROUP',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'c2a83b05-e05e-40e7-bfa9-8fd5835e4b94',
                                code: 'DMY_GROUP_TOTAL_TURNOVER',
                                label: 'mrc.mdw.totalturnover',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'a0b9c878-bb8e-435f-8385-a5c5695f25fc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710014Z',
                        value: 'example of string value',
                        textValue: null,
                        countryField: {
                            id: '5f36fbc7-c1c7-44df-a8df-fc0ea90c42b7',
                            mandatory: false,
                            validation: 'maxLength255',
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'eef491e5-466d-4f97-bf17-ea68208d1a73',
                                code: 'DMY_CUSTOMER_COMPANY_NAME',
                                label: 'mrc.customerdetails.fields.companyname',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '99efe89f-f397-4050-8205-26f8d080b2ef',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709854Z',
                        value: '20',
                        textValue: null,
                        countryField: {
                            id: '2e6cb78c-e5a9-49cc-93db-6a3e4db10990',
                            mandatory: false,
                            validation: 'isNumber',
                            level: 'GROUP',
                            order: 3,
                            section: 'CREDIT_DATA',
                            reusable: false,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'bbc8c7e4-b9d6-46b4-844b-9eca65829773',
                                code: 'DMY_GROUP_LIMIT_AVERAGE',
                                label: 'mrc.limit.average',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'b67a7132-4b25-4fd1-8408-88558a164711',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709889Z',
                        value: '2',
                        textValue: null,
                        countryField: {
                            id: 'ede80e37-7718-46b0-8566-eddcef681161',
                            mandatory: false,
                            validation: null,
                            level: 'GROUP',
                            order: 4,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: '1|2|3|4|5',
                            field: {
                                id: 'a0ca6a5a-2783-448f-a9c7-aefcd5b23f75',
                                code: 'DMY_GROUP_GENERAL_RATING',
                                label: 'mrc.recommendations.rating',
                                type: 'RADIOBUTTON',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
                title={'additional.fields.group.title'}
            />
        );
    })
    .add('editable customerAdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: '05c4e967-e4db-4f36-91e5-69f1f970e8e4',
                        requestId: 'a9a6f85e-6180-42d1-ad5e-cc26b8f78d0c',
                        country: 'DE',
                        storeNumber: '15',
                        customerNumber: '116102',
                        groupId: '951951951951951',
                        creationTimestamp: '2020-06-19T13:59:02.313021Z',
                        value: '9',
                        textValue: null,
                        countryField: {
                            id: 'cc8cc3b2-da48-4962-adda-d208ab063856',
                            mandatory: false,
                            validation: 'isPercentage',
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'f7722295-085b-41ff-bd5a-981a07220b9e',
                                code: 'percprofitability',
                                label: 'percentage.profitability',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'fb0a75ff-0d99-4571-95db-161e90e0e107',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710184Z',
                        value: 'Computer_peripheral_equipment|Recreational_Products_Toys',
                        textValue: null,
                        countryField: {
                            id: 'e3992ab5-425c-45ed-99f6-c82f63ea9be0',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 5,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options:
                                'Oil_Gas_Production|Computer_peripheral_equipment|Recreational_Products_Toys|Electric_Utilities_Central|Package_Goods_Cosmetics|EDP_Services|Commercial_Banks|Metal_Fabrications|Industrial_Machinery_Components',
                            field: {
                                id: '2bcc9833-1aa8-40ed-8b31-985a28fa906e',
                                code: 'DMY_CUSTOMER_INDUSTRY',
                                label: 'mrc.customerdetails.industry',
                                type: 'DROPDOWN',
                            },
                        },
                    },
                    {
                        id: '98e541fc-ca74-48d1-8072-e2d00dce5238',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710225Z',
                        value: 'true',
                        textValue: null,
                        countryField: {
                            id: 'cb45aef4-bbe7-4f1d-95d2-ece99a148a1d',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 6,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'a6c4d42a-43db-400b-a515-077a327be619',
                                code: 'DMY_CUSTOMER_IS_PROFITABLE',
                                label: 'mrc.customer.is.profitable',
                                type: 'CHECKBOX',
                            },
                        },
                    },
                    {
                        id: '173747f4-b76d-4fa6-a5c3-a009cab5e315',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710143Z',
                        value: '2020-12-31',
                        textValue: null,
                        countryField: {
                            id: 'f017993c-9338-45c4-abfb-6c61f407f4c3',
                            mandatory: false,
                            validation: 'isDateInFuture',
                            level: 'CUSTOMER',
                            order: 4,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'f47f3f98-a953-4c2b-b4a0-b050fbd0da92',
                                code: 'DMY_CUSTOMER_CONTRACT_EXPIRATION_DATE',
                                label: 'mrc.attachments.fields.contract.expiration_date',
                                type: 'DATE',
                            },
                        },
                    },
                    {
                        id: 'a0b9c878-bb8e-435f-8385-a5c5695f25fc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710014Z',
                        value: 'example of string value',
                        textValue: null,
                        countryField: {
                            id: '5f36fbc7-c1c7-44df-a8df-fc0ea90c42b7',
                            mandatory: false,
                            validation: 'maxLength255',
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'eef491e5-466d-4f97-bf17-ea68208d1a73',
                                code: 'DMY_CUSTOMER_COMPANY_NAME',
                                label: 'mrc.customerdetails.fields.companyname',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '8289cfb7-bf5e-410c-b2c0-958edb5edb67',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710068Z',
                        value: 'sample@example.com',
                        textValue: null,
                        countryField: {
                            id: '9789dade-f6e7-4e72-ae24-7f219d80d40f',
                            mandatory: false,
                            validation: 'isEmail',
                            level: 'CUSTOMER',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '12de83a4-1b56-482b-b283-a1209ca9e71e',
                                code: 'DMY_CUSTOMER_EMAIL',
                                label: 'mrc.customerdetails.fields.email',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '2c0e2a7d-9c08-46b4-8f17-e0582aa5b9df',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710102Z',
                        value: '+49 30 901820',
                        textValue: null,
                        countryField: {
                            id: '1e6519b8-f7d5-4698-8b62-bbaba20820ba',
                            mandatory: false,
                            validation: 'isPhone',
                            level: 'CUSTOMER',
                            order: 3,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'b43c8585-2a4f-4055-a2b5-3b174d3a30f4',
                                code: 'DMY_CUSTOMER_PHONE',
                                label: 'mrc.customerdetails.fields.phone',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '04388f3a-aa27-487a-9018-f863f0d2cb71',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709738Z',
                        value: '35',
                        textValue: null,
                        countryField: {
                            id: '0490c4a8-d46c-4ca4-968c-e0924e4eca75',
                            mandatory: false,
                            validation: 'isPercentage',
                            level: 'CUSTOMER',
                            order: 7,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '9ec81e6b-9e61-4663-8069-cd62c701fe04',
                                code: 'percprofitability',
                                label: 'mrc.topmanagement.profitability',
                                type: 'NUMBER',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
                editable={true}
            />
        );
    })
    .add('not editable customerAdditionalFields', () => {
        return (
            <AdditionalFieldsSection
                requestFields={[
                    {
                        id: '05c4e967-e4db-4f36-91e5-69f1f970e8e4',
                        requestId: 'a9a6f85e-6180-42d1-ad5e-cc26b8f78d0c',
                        country: 'DE',
                        storeNumber: '15',
                        customerNumber: '116102',
                        groupId: '951951951951951',
                        creationTimestamp: '2020-06-19T13:59:02.313021Z',
                        value: '9',
                        textValue: null,
                        countryField: {
                            id: 'cc8cc3b2-da48-4962-adda-d208ab063856',
                            mandatory: false,
                            validation: 'isPercentage',
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'f7722295-085b-41ff-bd5a-981a07220b9e',
                                code: 'percprofitability',
                                label: 'percentage.profitability',
                                type: 'NUMBER',
                            },
                        },
                    },
                    {
                        id: 'fb0a75ff-0d99-4571-95db-161e90e0e107',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710184Z',
                        value: 'Computer_peripheral_equipment|Recreational_Products_Toys',
                        textValue: null,
                        countryField: {
                            id: 'e3992ab5-425c-45ed-99f6-c82f63ea9be0',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 5,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options:
                                'Oil_Gas_Production|Computer_peripheral_equipment|Recreational_Products_Toys|Electric_Utilities_Central|Package_Goods_Cosmetics|EDP_Services|Commercial_Banks|Metal_Fabrications|Industrial_Machinery_Components',
                            field: {
                                id: '2bcc9833-1aa8-40ed-8b31-985a28fa906e',
                                code: 'DMY_CUSTOMER_INDUSTRY',
                                label: 'mrc.customerdetails.industry',
                                type: 'DROPDOWN',
                            },
                        },
                    },
                    {
                        id: '98e541fc-ca74-48d1-8072-e2d00dce5238',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710225Z',
                        value: 'true',
                        textValue: null,
                        countryField: {
                            id: 'cb45aef4-bbe7-4f1d-95d2-ece99a148a1d',
                            mandatory: false,
                            validation: null,
                            level: 'CUSTOMER',
                            order: 6,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'a6c4d42a-43db-400b-a515-077a327be619',
                                code: 'DMY_CUSTOMER_IS_PROFITABLE',
                                label: 'mrc.customer.is.profitable',
                                type: 'CHECKBOX',
                            },
                        },
                    },
                    {
                        id: '173747f4-b76d-4fa6-a5c3-a009cab5e315',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710143Z',
                        value: '2020-12-31',
                        textValue: null,
                        countryField: {
                            id: 'f017993c-9338-45c4-abfb-6c61f407f4c3',
                            mandatory: false,
                            validation: 'isDateInFuture',
                            level: 'CUSTOMER',
                            order: 4,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'f47f3f98-a953-4c2b-b4a0-b050fbd0da92',
                                code: 'DMY_CUSTOMER_CONTRACT_EXPIRATION_DATE',
                                label: 'mrc.attachments.fields.contract.expiration_date',
                                type: 'DATE',
                            },
                        },
                    },
                    {
                        id: 'a0b9c878-bb8e-435f-8385-a5c5695f25fc',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710014Z',
                        value: 'example of string value',
                        textValue: null,
                        countryField: {
                            id: '5f36fbc7-c1c7-44df-a8df-fc0ea90c42b7',
                            mandatory: false,
                            validation: 'maxLength255',
                            level: 'CUSTOMER',
                            order: 1,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'eef491e5-466d-4f97-bf17-ea68208d1a73',
                                code: 'DMY_CUSTOMER_COMPANY_NAME',
                                label: 'mrc.customerdetails.fields.companyname',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '8289cfb7-bf5e-410c-b2c0-958edb5edb67',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710068Z',
                        value: 'sample@example.com',
                        textValue: null,
                        countryField: {
                            id: '9789dade-f6e7-4e72-ae24-7f219d80d40f',
                            mandatory: false,
                            validation: 'isEmail',
                            level: 'CUSTOMER',
                            order: 2,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '12de83a4-1b56-482b-b283-a1209ca9e71e',
                                code: 'DMY_CUSTOMER_EMAIL',
                                label: 'mrc.customerdetails.fields.email',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '2c0e2a7d-9c08-46b4-8f17-e0582aa5b9df',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.710102Z',
                        value: '+49 30 901820',
                        textValue: null,
                        countryField: {
                            id: '1e6519b8-f7d5-4698-8b62-bbaba20820ba',
                            mandatory: false,
                            validation: 'isPhone',
                            level: 'CUSTOMER',
                            order: 3,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: 'b43c8585-2a4f-4055-a2b5-3b174d3a30f4',
                                code: 'DMY_CUSTOMER_PHONE',
                                label: 'mrc.customerdetails.fields.phone',
                                type: 'TEXT',
                            },
                        },
                    },
                    {
                        id: '04388f3a-aa27-487a-9018-f863f0d2cb71',
                        requestId: '1',
                        country: 'DE',
                        storeNumber: '30',
                        customerNumber: '1234567',
                        groupId: null,
                        creationTimestamp: '2020-06-19T14:20:49.709738Z',
                        value: '35',
                        textValue: null,
                        countryField: {
                            id: '0490c4a8-d46c-4ca4-968c-e0924e4eca75',
                            mandatory: false,
                            validation: 'isPercentage',
                            level: 'CUSTOMER',
                            order: 7,
                            section: 'CREDIT_DATA',
                            reusable: true,
                            endTimestamp: null,
                            country: 'DE',
                            options: null,
                            field: {
                                id: '9ec81e6b-9e61-4663-8069-cd62c701fe04',
                                code: 'percprofitability',
                                label: 'mrc.topmanagement.profitability',
                                type: 'NUMBER',
                            },
                        },
                    },
                ]}
                onChange={(value) => console.log(value)}
                onBlur={(value) => console.log(value)}
            />
        );
    });
